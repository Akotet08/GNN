...
print(dataset.graph['edge_index'].shape, dataset.graph['node_feat'].shape, dataset.label.shape)
...
elif args.dataset in ('book_crossing'):
    # criterion is BPRLoss
    pass
else:
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if args.dataset in ('yelp-chi', 'deezer-europe', 'twitch-e', 'fb100', 'ogbn-proteins'):
            ...
            # this is a recommendation dataset
            ...
            for i in usr_index:
                ...
                for j in item_index:
                    item_emb = out[j]
                    negative_sample_idx = dataset.graph['neg_sample_indexes'][i]
                    neg_emb = out[negative_sample_idx]

                    pos_score = torch.sum(usr_emb * item_emb)
                    neg_score = torch.sum(usr_emb * neg_emb, dim=1)
        ...
        if epoch % args.eval_step == 0:
            if dataset.name == 'book_crossing':
                result = evaluate(model, dataset, split_idx, None, None, args)
            else:
            ...
            print(f'Epoch: {epoch:02d}, '
                  f'Loss: {loss:.4f}, '
                  f'NCDG@5: {result[0]:.4f}, '
                  f'NCDG@10: {result[1]:.4f}, '
                  f'head-NCDG@5: {result[2]:.4f}', 
                  f'tail-NCDG@5: {result[3]:.4f}')
