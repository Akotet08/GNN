...
from dataset import load_recommendation_dataset
...
if __name__ == '__main__':
    # test the implementation of NodeFormerConv
    
    dataset_name = 'book_crossing'
    dataset = load_recommendation_dataset(dataset_name)
    ...
    model = NodeFormer(in_channels=node_feat.shape[1], hidden_channels=hidden_channels, out_channels=out_channels,
                        num_layers=2, num_heads=num_heads, dropout=0.0, kernel_transformation = softmax_kernel_transformation,)
    model.reset_parameters()
    
    out, link_loss= model(node_feat, [edge_index], tau)
    print(out.shape, link_loss)
